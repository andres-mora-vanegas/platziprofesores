...
public class JwtUtil {
	...
	// Metodo para crear el JWT y enviarlo al cliente en el header de la
	// respuesta
	...
	static Authentication getAuthentication(HttpServletRequest request, String[] roles) throws IOException {
		...
		if (token != null) {
			String user = Jwts.parser().setSigningKey(KEYSECRET).parseClaimsJws(token) // este
																						// metodo
																						// es
																						// el
																						// que
			...
			// UsernamePasswordAuthenticationToken sin password, pero debemos
			// consultar nuevamente los roles
			// seguramente hay una mejor forma de implementar roles, pero no
			// esta del todo mal volver a consultarlos, puesto que si hubo un
			// cambio
			// en la asignación de roles a un usuario, se le otorgara o se le
			// denegara el acceso de forma automatica, y no hay que esperar al
